...
void descompila(AST* node, FILE * output) {
    ...
    switch (node->type) {
        // case AST_ADD: fputs(" + ", output); descompila(node->son[i]); break;
        // case AST_SUB: fputs(" - ", output); descompila(node->son[i], level+1); break;
        // case AST_EXPR_PARENTESES: descompila(node->son[0], level+1); fputs(")", output); break;
        // case AST_LIST_DECLARATIONS: fputs(";", output); break;
        // case AST_ATTR_INTE: descompila(node->son[i], level); break;
        default: break;
    }
}